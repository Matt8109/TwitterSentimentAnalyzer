...
namespace Oclumen.Core.LanguageProcessing
{
    public class EnglishWordStemmer
    {
        ...
        private static readonly char[] Vowels = new char[6] { 'a', 'e', 'i', 'o', 'u', 'y' };

        private static readonly char[] NotShortSyllableNonVowels = new char[9] { 'a', 'e', 'i', 'o', 'u', 'y', 'w', 'x', 'Y' };

        private static readonly string[] DoubleChars = new string[9] { "bb", "dd", "ff", "gg", "mm", "nn", "pp", "rr", "tt" };
        ...
        public EnglishWordStemmer(string input)
        {
            Create(input);
            GenerateStem();
        }
        ...
        internal static EnglishWordStemmer CreateWithR1R2(string text)
        {
            EnglishWordStemmer result = CreateForTest(text);
            result._r1 = CalculateR(result.Stem, 0);
            result._r2 = CalculateR(result.Stem, result._r1.Start);
            return result;
        }

        internal static EnglishWordStemmer CreateForTest(string text)
        {
            var word = new EnglishWordStemmer();
            word.Create(text);
            return word;
        }
